...
public class EmpresaDao {
    ...
    public void salvar(EmpresaBeans mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("insert into empresa (nome, localizacao, contacto, email) values(?,?,?,?)");
    ...
    public void editar(EmpresaBeans mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("update empresa set nome=?, localizacao=?, contacto=?, email=? where id=?");
            pst.setString(1, mod.getNome());
            pst.setString(2, mod.getLocalizacao());
            pst.setString(3, mod.getContacto());
            pst.setString(4, mod.getEmail());
    ...
    public void excluir(EmpresaBeans mod){
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("delete from empresa where id=?");
    ...
    public EmpresaBeans buscaEmpresa(EmpresaBeans mod) {
        ...
        conex.executaSql("select * from empresa where nome like '%" + mod.getPesquisa() + "%'");
           ...
           try { 
            ...
            mod.setLocalizacao(conex.rs.getString("localizacao"));
